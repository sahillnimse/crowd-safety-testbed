...
**Throughput is roughly 2.4 fps at 1080p on CPU with visualisation on and the person detector off** — not real time. Metrics and overlays run at source resolution while the field only carries information at the compute resolution, so the cheapest lever is a smaller source; `visualise=False` removes the rendering term entirely for headless deployments.

Since the flow-overlay scoping fix, `people_overlay: true` is the **default**: both overlays are masked to confirmed moving people, which costs the RT-DETRv2 detector **+449 ms/frame amortised** (304 ms median / 448 ms mean at 3×3 tiling, `detect_every=5`, measured on a GTX 1650 — CPU runs pay more and that combination has not been measured). The 2.4 fps figure above is kept as the explicit **detector-off reference point**; set `people_overlay: false` (or `--no-people` on `run_flow_analysis.py`) to get it back.
