...
class PeopleOverlay:
    ...
    def confirmed_boxes(self) -> list[tuple[float, float, float, float, float, float]]:
        """
        (x1, y1, x2, y2, vx, vy) per confirmed track, source px and px/frame.

        Read-only snapshot of the state ``draw()`` renders. Consumers — the
        dense-flow divergence heatmap masks itself to these boxes — need where
        each person IS and whether they are MOVING; both already live on the
        track as the smoothed box and the smoothed median flow velocity, so no
        second detector or re-sampling of the field is involved.
        """
        return [
            (float(t.box[0]), float(t.box[1]),
             float(t.box[2]), float(t.box[3]),
             float(t.vx), float(t.vy))
            for t in self._tracks.values() if t.confirmed
        ]
    ...
    def draw(self, frame: np.ndarray, in_place: bool = False) -> np.ndarray:
        """Boxes and per-person arrows.

        Copies the frame unless ``in_place``; pass that only when the caller
        owns a private buffer (the dense-flow pipeline does, from the first
        alpha blend onward). Default preserves the never-mutate contract.
        """
