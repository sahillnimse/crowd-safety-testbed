...
VENDOR_DIR = _REPO_ROOT / "vendor" / "apgcc"
...
def _vendored_on_path():
    """Put the correct directories on sys.path so all APGCC imports resolve.

    Two separate path entries are required:

    * ``_REPO_ROOT`` — needed for ``from vendor.apgcc.<mod> import …`` calls
      made in *this* file (e.g. build_apgcc imports vendor.apgcc.config and
      vendor.apgcc.models).
    * ``VENDOR_DIR`` (= _REPO_ROOT/vendor/apgcc) — needed for bare
      ``from util.misc import …`` calls made *inside* the vendored source
      (e.g. vendor/apgcc/models/APGCC.py imports util.misc, which lives at
      vendor/apgcc/util/misc.py).

    When the webapp server runs, both are already on sys.path via uvicorn's
    working directory + conftest; for standalone scripts neither may be.
    """
    ...
    to_add = [str(_REPO_ROOT), str(VENDOR_DIR)]
    added = [p for p in to_add if p not in sys.path]
    for p in added:
        sys.path.insert(0, p)
    try:
        yield
...
def _disable_hardcoded_imagenet_init() -> None:
    ...
    try:
        from vendor.apgcc.models.backbones import vgg as _vgg_mod  # type: ignore
...
def build_apgcc(config: str | Path = "shha", imagenet_init: bool = False) -> nn.Module:
    ...
    with _vendored_on_path():
        ...
        from vendor.apgcc.config import cfg as _cfg, merge_from_file  # type: ignore
        from vendor.apgcc.models import build_model  # type: ignore
