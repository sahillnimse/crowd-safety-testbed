...
COLOR_MAP = {
    ...
    # "<metric>_<severity>" convention as the dense-flow engine's.
    "head_moving":                (140, 200, 0),   # teal-green - tracked head moving
    "head_stopped":               (0, 40, 220),    # red - stationary head
    "crowd_density_warning":      (0, 165, 255),   # orange
    "crowd_density_critical":     (0, 0, 200),     # red
    "crowd_compression_warning":  (140, 0, 220),   # purple
    "crowd_compression_critical": (90, 0, 150),    # dark purple - stampede range
    "counter_flow_warning":       (0, 215, 255),   # amber
    "capacity_warning":           (60, 160, 255),  # amber-orange
    "dm_frame_metrics":           (200, 200, 200), # grey - telemetry row, not an alert
...
def draw_frame(frame, detections: list[Detection]):
    ...
    for d in (detections or []):
        ...
        if d.bbox:
            ...
            if isinstance(d.extra, dict):
                if d.extra.get("plate_display"):
                    text = f"{d.label}: {d.extra['plate_display']}"
                elif d.extra.get("track_id") is not None:
                    text = f"#{d.extra['track_id']} {text}"

            (tw, th), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
            text_y = max(y1 - 6, th + 4)
            cv2.rectangle(annotated, (x1, text_y - th - 2), (x1 + tw + 4, text_y + baseline), (0, 0, 0), -1)
            cv2.putText(annotated, text, (x1 + 2, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)
        else:
            # clip-level detection with no bbox
            y = 30 + banner_slot * 26
            banner_slot += 1
            text = f"[{d.model_name}] {d.label} {d.confidence:.2f}"
    ...
    cv2.putText(annotated, "LIVE", (w - 70, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
