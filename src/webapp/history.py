...
from webapp.jobs import (POSITIVE_LABELS, RUNS_DIR, RUN_CSV,
                         RUN_JSON, RUN_REPORT, RUN_SUMMARY, find_run_video)
...
def _summarize(path: str) -> dict:
    ...
    if stored_summary is None:
        ...
        # Also persist summary.json for fast reload. A failure here is not
        # fatal (the summary above still serves this request) but must not be
        # silent: the cache key is (mtime, size), so without the file every
        # poll re-parses the full multi-MB detection log, and the UI shows a
        # "Summary Stats" link that 404s with nothing in any log explaining
        # why.
        try:
            ...
            logger.warning(
                "Could not write %s: %s — history polling will keep "
                "re-parsing detections.json until this succeeds.",
                summary_path, exc,
            )
    ...
    if not os.path.isfile(report_path):
        try:
            ...
            logger.warning(
                "Could not generate %s: %s — the modal's standalone-report "
                "link will be absent for this run.", report_path, exc,
            )
    ...
    for video in os.listdir(RUNS_DIR):
        ...
        for model_key in os.listdir(video_dir):
            ...
            by_video[video].append({
                ...
                # Architecture text from the registry. None (not a crash) for
                # model keys that have since been removed or renamed, so old
                # history directories still render.
                "algorithm": spec.blurb if spec else None,
                "status": "done",
                "progress": 1.0,
                ...
                # By stem, not by a hardcoded "annotated.mp4": a machine
                # without ffmpeg writes annotated.avi, and matching only the
                # mp4 name reported "no annotated video" for a file that was
                # sitting right there.
                "annotated": ((lambda n: f"{rel}/{n}" if n else None)(
                    find_run_video(model_dir))),
            })
