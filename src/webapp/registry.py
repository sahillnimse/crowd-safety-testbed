...
def _needs_api_key(note: str = "Roboflow-hosted model; inference runs on "
    ...
    Reports `blocked` without a key. This used to return `ready`
    unconditionally, justified by a hardcoded fallback key in
    roboflow_combined.py -- that key is gone, and all three wrappers now
    raise from load() when ROBOFLOW_API_KEY is unset. So the UI offered three
    models as available and each one failed partway into a job, which is the
    exact failure this module exists to prevent.

    Read at call time, not at import: the server is long-lived, and a key
    exported after startup should be picked up by the next listing rather
    ...
    def check():
        ...
        return False, "blocked", (
            "ROBOFLOW_API_KEY is not set, so this hosted model cannot run. "
            "Get a key at roboflow.com (Workspace Settings -> Private API "
            "Key) and set it as an environment variable before starting the "
            "server. " + note
        )
    return check
...
def _dm_count_check():
    """Blocked unless the DM-Count checkpoint is on disk.

    Unlike the auto-download models, DM-Count weights have no pip/HF source
    wired up here — they are copied out of a local model collection — so a
    missing checkpoint must visibly block rather than fail mid-job.
    """
    def check():
        # Imported lazily: registry stays import-light for instant listing.
        from models.dm_count.infer import find_weights
        if find_weights():
            return True, "ready", (
                "ShanghaiTech-A density-map checkpoint found. Runs on CPU at "
                "the configured long-side cap; speeds are px/frame unless a "
                "camera homography is configured.")
        return False, "blocked", (
            "No DM-Count checkpoint. Copy model_sh_A.pth into "
            "'src/models/dm_count/weights/' or 'ML Models/dm_count/'.")
    return check
...
MODELS: list[ModelSpec] = [
    ...
    ModelSpec("crowd_motion_monitor", "Crowd Motion Monitor", "crush",
              ...
              "RT-DETRv2 person detector fused with APGCC head points (recovers "
              "people RT-DETRv2 misses) + IoU tracker + Farneback flow.",
              check=lambda: (True, "ready",
                             "RT-DETRv2 person detector + APGCC head-point "
                             "fusion + IoU tracker + Farneback dense flow. "
                             "RT-DETRv2 weights auto-download from HuggingFace "
                             "Hub on first run; APGCC loads its SHHA checkpoint "
                             "from models/head_count/weights/."),
              comparable_threshold=False,
              tags=["flow", "cpu", "tracker"]),
    ModelSpec("dm_count_crowd", "DM-Count Crowd Metrics", "crush",
              "Density-map head counter (VGG19, ShanghaiTech-A) + BYTE-style "
              "point tracking. Reports ten crowd-risk metrics per frame "
              "(density, Helbing pressure, divergence, entropy, counter-flow %, "
              "stop-and-go, oscillation) with rule-based alerts.",
              check=_dm_count_check(),
              comparable_threshold=False,
              tags=["density", "tracker", "flow", "cpu"]),
]
...
def build_model(key: str, device: Optional[str],
    ...
    factories = {
        ...
        "dense_flow": lambda: _build_dense_flow(device, camera_id=camera_id),
        "crowd_motion_monitor": lambda: _build_crowd_motion_monitor(device, video_name=video_name, threshold=threshold),
        "dm_count_crowd": lambda: _build_dm_count(device, video_name=video_name, threshold=threshold),
...
def _build_dense_flow(device, camera_id: str = "default"):
...
def _build_dm_count(device, video_name: str = "run", threshold=None):
    from models.dm_count import DMCountCrowdMonitor
    stem = os.path.splitext(os.path.basename(video_name or "run"))[0]
    output_dir = os.path.join(PROJECT_ROOT, "outputs", "annotated")
    os.makedirs(output_dir, exist_ok=True)
    kw = {"device": device, "video_name": stem, "output_dir": output_dir}
    # Same threshold convention as crowd_motion_monitor: comparable_threshold=
    # False keeps the run-wide threshold away from this model, while an
    # explicit per-model value maps to the stopped-speed floor (px/frame).
    if threshold is not None:
        kw["stopped_speed"] = float(threshold)
    return DMCountCrowdMonitor(**kw)
