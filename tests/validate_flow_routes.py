...
This is the empirical counterpart to tests/validate_flow.py.  That suite
...
-----
  ...
  python tests/validate_flow_routes.py --source test_videos/Umbrellas.mp4
  ...
  python tests/validate_flow_routes.py --routes b --selftest-cross-camera
...
def _load_cfg(config_path: str) -> dict:
    if not os.path.exists(config_path):
        return {}
    sys.path.insert(0, str(_SRC_DIR))   # ensure config_io is importable
    from config_io import load_yaml_dict
    return load_yaml_dict(config_path).get("crowd_flow", {})
